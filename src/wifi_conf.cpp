...
void initWifi() {
  ...
  wifiStatusCharacteristic->addDescriptor(new BLEDescriptor(WIFI_STATUS_CHARACTERISTIC_UUID));
    ...
    if (xQueueReceive(cmdQ, &cmd, 100)) { 
      switch (cmd) {
        case WIFI_CMD_SCAN: { 
            ...
            const char *currentStatus = wifiStatusCharacteristic->getValue().c_str();
            ...
            for (int i = 0; i < n; ++i) {
              ...
              foundWifiCharacteristic->setValue(WiFi.SSID(i).c_str());
